...
fn print_oneway_solution(session: &mut Session, eff: &str, stats: &OneWayStats) {
    let k = stats.k;
    let df_error = stats.df_error;
    let mse = stats.mse;
...
struct MultiwayFit {
    factors: Vec<Factor>,
    term_factor_idxs: Vec<Vec<usize>>,
    col_specs: Vec<Vec<Vec<(usize, usize)>>>,
    term_df: Vec<usize>,
    term_labels: Vec<String>,
    full_design: Vec<Vec<f64>>,
    y: Vec<f64>,
    ncols: usize,
    y_bar: f64,
    sst: f64,
...
fn print_multiway_anova_and_fit(session: &mut Session, dep_var: &str, fit: &MultiwayFit) {
    let &MultiwayFit {
        y_bar,
        sst,
        sse_full,

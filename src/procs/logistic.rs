...
fn wald_global_test(beta: &[f64], var_beta: &[Vec<f64>], nb_cols: usize) -> Result<f64> {
...
fn print_convergence_status(session: &mut Session, converged: bool) {
    session.listing.blank();
...
fn print_ordinal_model_information(
    session: &mut Session,
    in_libref: &str,
    in_table: &str,
    resp_name: &str,
...
fn print_ordinal_response_profile(
    session: &mut Session,
    resp_name: &str,
    cat_vec: &[usize],
    freq_vec: &[f64],
...
fn print_ordinal_parameter_estimates(
    session: &mut Session,
    design: &Design,
    theta: &[f64],
    se: &[f64],

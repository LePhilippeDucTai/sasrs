...
impl Runner {
    ...
    pub(super) fn exec_set_interleave(&mut self) -> Result<Flow> {
        let Some(input) = &self.input else {
            return Err(SasError::runtime("SET statement without input data."));
        };
    ...
    pub(super) fn exec_merge(&mut self) -> Result<Flow> {
        let Some(input) = &self.input else {
    ...
    /// un dataset) → ERROR.
    pub(super) fn build_merge_plan(&mut self) -> Result<Vec<MergeObs>> {
        let input = self.input.as_ref().unwrap();
    ...
    pub(super) fn prefilter(&mut self) -> Result<()> {
        let Some(input) = &self.input else {
            return Ok(());
        };
        for (d, ds) in input.datasets.iter().enumerate() {

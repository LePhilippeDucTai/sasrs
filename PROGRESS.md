...
- [x] M15.3 — date/heure : DATEPART, TIMEPART (rem_euclid pour datetimes négatifs), DATETIME (figé=0 sous `--deterministic` via `EvalCtx.deterministic` câblé depuis `Session`), HMS, DHMS, HOUR/MINUTE/SECOND (time ou datetime, SECOND garde la fraction), YRDIF (ACT/ACT, ACT/360, ACT/365, 30/360 US/NASD), DATDIF (ACT/ACT, 30/360), JULDATE/DATEJUL (réciproques YYDDD/YYYYDDD). Réutilise les helpers calendaires existants. Différés (non au DISPATCH) : NLDATE/NLDATEMN/INTFMT/INTSHIFT ; basis `AGE` → missing + erreur claire. +26 tests (1168 lib + snapshot, 0 `.snap.new`, 0 warning)

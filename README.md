...
| `SGPLOT` | 🟡 | image rendering infrastructure (M29.1) via the optional `graphics` feature (`plotters`, PNG + SVG); `DrawingSpec`/`draw_to_file` for SCATTER/SERIES/VBAR/HISTOGRAM. Without `--features graphics` the default build is byte-identical (a NOTE is emitted, no image). Full statement parsing/plotting arrives in M29.2 | `SCATTER`/`SERIES`/`VBAR`/`HBAR`/`HISTOGRAM`/`DENSITY`/`VBOX`/`REG`/`LOESS`, `XAXIS`/`YAXIS`, `BY` (M29.2) |
...
| `ODS GRAPHICS` | 🟡 | `ON`/`OFF`, `WIDTH=`/`HEIGHT=`/`IMAGEFMT=(PNG\|SVG)`/`IMAGENAME=`/`RESET=` (per-statement); image rendering via the optional `graphics` feature (`plotters`). Default build is byte-identical (NOTE only, no image) |

...
| `UNIVARIATE` | 🟡 | `DATA=`, `NOPRINT`, `NORMAL`/`NORMALTEST`; `VAR` (`/ normal`), `WEIGHT`, `BY` (parsed), `OUTPUT` (parsed). Report: Moments, Basic Measures, Quantiles, Extreme Obs, Tests for Normality. Plots wired to ODS GRAPHICS: `HISTOGRAM`/`QQPLOT` → PNG/SVG under `--features graphics` (`univar_{N}`, histogram & normal-QQ scatter), else NOTE; nothing emitted when ODS GRAPHICS off | `PROBPLOT`/`CDFPLOT`/`PPPLOT` rendering (NOTE-deferred), weighted quantiles/extremes |
...
| `REG` | 🟡 | `DATA=`; `MODEL dep = x1 x2 … / NOPRINT`; `OUTPUT OUT= PREDICTED= RESIDUAL=`; OLS via QR (intercept only), ANOVA table, R²/Adj R²/F/t-tests, parameter estimates with SE, listwise missing deletion. Diagnostics wired to ODS GRAPHICS: automatic residuals-vs-predicted scatter after each `MODEL` → PNG/SVG under `--features graphics` (`reg_{N}`), else NOTE; `PLOTS` statement parsed → NOTE-deferred | `NOINT`, `TEST` statement, CLM/CLI prediction intervals, `BY`, multiple MODEL statements per run, full `PLOTS=` diagnostic panel |

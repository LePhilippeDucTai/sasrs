...
| `1` | Run completed, but at least one WARNING was logged (e.g. a recognized-but-deferred display option, non-convergence of an iterative fit, `PROC PRINTTO` routing not supported). A requested exit code (`%ABORT RETURN n`) is raised to 1 when errors were counted. |
...
| `MEANS` / `SUMMARY` | ✅ | `DATA=`, `NOPRINT`, `PRINTALLTYPES`, stat keywords (`N NMISS MEAN STD MIN MAX SUM RANGE STDERR CV MEDIAN CLM LCLM UCLM` + percentiles `P1 P5 P10 P20 P25 P30 P40 P50 P60 P70 P75 P80 P90 P95 P99 Q1 Q3 QRANGE`, Definition 5); `CLASS`, `VAR`, `BY`, `WEIGHT`, `WAYS`, `TYPES`, `OUTPUT OUT= stat(var)=name` | `MAXDEC=`, `NWAY`, `MISSING`, `ORDER=`, `ID`, multi-label formats. Under `WEIGHT`: weighted N/sum-of-weights, mean, std, min, max, sum — but `MEDIAN` and the percentile statistics (`P1`…`P99`, `Q1`, `Q3`, `QRANGE`) remain computed **unweighted** on the usable values (weighted quantiles deferred; using the weighted mean with unweighted quantiles), and an uncomputable OUTPUT statistic is an ERROR rather than a silent missing column |
...
| `COMPARE` | ✅ | `BASE=`, `COMPARE=`, `OUT=`, `NOVALUES`, `BRIEFSUMMARY` | `CRITERION=`, `METHOD=`, `BRIEF`, `LISTALL`, `OUTBASE=`/`OUTCOMP=`/`OUTDIF=`/`OUTNOEQUAL=`/`OUTPERCENT=`, `MAXPRINT=` and unknown options are **ERRORs** (they could change the comparison); `ID`, `VAR`/`WITH` |
...
| `PRINTTO` | 🟡 | `LOG=`, `PRINT=`, `NEW`, reset (options stored) | actual file routing: `LOG=`/`PRINT=` produce an explicit **WARNING** ("routing not supported until J07-P5"), exit code 1, default destinations unchanged; real routing deferred |

...
| `ODS OUTPUT` | 🟡 | generic capture by ODS object name (typed values, diagonal union across TABLES/BY panels; registry lives for one procedure step; unknown name → SAS `WARNING: Output '…' was not created` at step end): `OneWayFreqs` (FREQ), `Moments`/`BasicMeasures` (UNIVARIATE), `Summary` (MEANS), `TTest` (TTEST) | other output objects (named incrementally as procs are wired), capture of bare SQL `SELECT` |
| `ODS SELECT`/`EXCLUDE` | 🟡 | object-name lists, `ALL`/`NONE` (`select none` ≡ `exclude all`); SAS lifecycle: named lists consumed at the end of the next procedure step (reset to `SELECT ALL`), `ALL`/`NONE` lists persist; `ODS OUTPUT` still captures excluded tables (display-only exclusion); `SELECT NONE`/`EXCLUDE ALL` suppresses the whole proc listing while keeping LOG/`OUT=`/captures. Named objects: `OneWayFreqs`/`OneWayChiSq` (FREQ), `Moments`/`BasicMeasures`/`TestsForNormality`/`Quantiles`/`ExtremeObs`/`MissingValues` (UNIVARIATE), `Summary` (MEANS) | per-destination lists (single global list), `(PERSIST)`, name filtering of not-yet-named tables (they always display except under `NONE`/`EXCLUDE ALL`) |
...
| User formats | 🟡 | `PROC FORMAT` `VALUE`/`INVALUE`/`PICTURE`; persistent per-libref catalogs via `LIB=` (JSON sidecar, loaded at `LIBNAME`); `CNTLOUT=`/`CNTLIN=` control datasets (round-trip; PICTURE excluded) |

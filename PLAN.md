...
| Bloc 0 — store de catalogue de format persistant : sidecar JSON par libref, `CNTLIN=`/`CNTLOUT=`, `FMTLIB`/`FMTSEARCH=` | M39 | 🔶 | **Sonnet** | moyen | **jalon courant** — M39.1 fait (sidecar + `LIB=` + round-trip) ; reprendre à M39.2 |
...
**Curseur (2026-08-11)** : M1–M38 terminés ; **jalon courant M39** (reprendre à M39.2) ;
